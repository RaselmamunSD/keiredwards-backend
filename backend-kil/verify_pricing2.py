import re

with open('c:/Rasel/keiredwards/backend-kil/apps/dashboard/templates/dashboard/custom_admin.html', 'r', encoding='utf-8') as f:
    content = f.read()

dc_start = content.find('<script type="text/x-dc"')
dc_end = content.find('</script>', dc_start) + len('</script>')
dc_script = content[dc_start:dc_end]

idx = dc_script.find('onSave')
if idx != -1:
    print(f"onSave found at {idx}:")
    print(dc_script[max(0,idx-50):idx+500])

idx = dc_script.find('saveChanges')
if idx != -1:
    print(f"saveChanges found at {idx}:")
    print(dc_script[max(0,idx-50):idx+500])

idx = dc_script.find('fetch(')
if idx != -1:
    print(f"fetch found at {idx}:")
    print(dc_script[max(0,idx-50):idx+500])

